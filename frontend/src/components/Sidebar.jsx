...
import { MessageSquare, Settings as SettingsIcon, X, Search, Clock, PanelLeftClose, LogOut, Trash2 } from 'lucide-react';
...
export default function Sidebar({ 
  ...
  const navItems = [
    ...
    { id: 'settings', label: 'Settings', icon: SettingsIcon, action: () => setActiveView('settings') },
    { id: 'clear', label: 'Clear History', icon: Trash2, action: () => {
    ...
    <>
      ...
      <aside className={`sidebar ${isMobileOpen ? 'mobile-open' : ''} ${isDesktopCollapsed ? 'desktop-collapsed' : ''}`}>
        <div className="sidebar-header">
          ...
          {!isDesktopCollapsed && (
            <button className="icon-btn desktop-close" onClick={toggleDesktopSidebar} title="Hide sidebar">
